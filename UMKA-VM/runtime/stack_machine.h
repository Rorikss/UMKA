...
class StackMachine {
  public:
    StackMachine(const auto &parser, const auto &jit_runner)
  ...
  private:
    ...
    void execute_command(const Command &cmd, StackFrame &current_frame, size_t current_offset) {
      ...
      auto BinaryOperationDecoratorWithApplier = [machine = this
          ](const std::string &op_name, auto f, auto applier) {
        auto [lhs, rhs] = machine->get_operands_from_stack(op_name);
        Entity result = applier(lhs, rhs, f);
        machine->create_and_push(result);
      };

      auto BinaryOperationDecorator = [BinaryOperationDecoratorWithApplier
          ](const std::string &op_name, auto f) {
      ...
      switch (cmd.code) {
        ...
        case CALL: {
          switch (cmd.arg) {
            case PRINT_FUN: call_void_proc([this](auto arg) { print(arg); });
              break;
            case LEN_FUN: call_value_proc([this](auto arg) { return len(arg); });
              break;
            case GET_FUN: {
            ...
            default: {
              ...
              if (auto iter = jit_functions.find(cmd.arg); iter != jit_functions.end()) {
               //TODO
                break;
              }
              if (profiler -> is_function_hot(cmd.arg)) {
                auto jitted_function = jit->optimize_function(cmd.arg);
                jit_functions[cmd.arg] = jitted_function;
                //TODO
                break;
              }
        ...
        default:
          throw std::runtime_error(
            "Unknown opcode: " + std::to_string(cmd.code) + " at " + std::to_string(
              current_offset));
      }
    }
    ...
    void printDebugParsedInfo() {
      ...
      for (int i = 0; i < commands.size(); ++i) {
        std::cout << i << " " << (long long) (commands[i].code) << " " << (long long) (commands[i].
          arg) << "\n";
      }
      std::cout << "\n";
    }
    ...
    std::unique_ptr<jit::JitRunner> jit;
